...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        if (not newEdge in self.edges) and (not reverse in self.edges) and not source == target:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        or {'a': 22, 'b': 22}
        """
        #Step 1: Creating recording all degrees
        vals = defaultdict(int)
        for i in self.edges:
            #Assuming that edges can self-direct
            vals[i[0]] += 1
            if not i[0] == i[1]:
                vals[i[1]] += 1
        ...
        #Step 3: Output
        return {k:v for k,v in vals.items() if v == highest}
        return NotImplemented
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        """
        url = "https://api.themoviedb.org/3/movie/" + movie_id + "/credits?language=en-US&api_key=" + self.api_key
        response = None
        ...
        if exclude_ids == None:
            exclude_ids = []
        if limit == None:
            limit = len(cast) + 100 #Paranoia constant
        for i in cast:
            ord = i['order']
            if ord >= limit:
                return out
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        url = "https://api.themoviedb.org/3/person/" + person_id + "/movie_credits?language=en-US&api_key=" + self.api_key
        response = None
        while response is None:
            try:
                response = urlopen(url)
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='8e1b388e86672b748d4c0ef4c8385ccf')
    t = tmdb_api_utils
    usedIds = [2975, '2975']
    ...
    counter = 0
    EXIT = 3
    #There is no actual need for a base building step. Just treat Lawrence Fishburn as a newly added Node and build 3 loops
    while counter < EXIT and len(ids) > 0:
        counter = counter + 1
        newIDs = []
        for id in ids:
            for movie in t.get_movie_credits_for_person(str(id), 8.0):
                foundList = t.get_movie_cast(str(movie['id']), 3, [])
                for found in foundList:
                    if not found['id'] in usedIds:
                        usedIds.append(found['id'])
                        newIDs.append(found['id'])
                        graph.add_node(id=str(found["id"]), name=found['name'].replace(',', ''))
                    graph.add_edge(str(id), str(found['id']))
